...
| `globals.css`           | —              | Global CSS. Brand design tokens, Tailwind v4 setup, `.container-brand` utility, scrollbar styling, marquee animation, selection colours, and **coffee steam keyframe animations** (`steamWisp1/2/3`, `.steam-wisp-1..5` utility classes used by the About page hero). |
...
All six components share the brand design system (`#0e0b08`/`#15110D` background, gold `#B8956A`, cream `#EDE3D0`) and use Framer Motion `whileInView` / `fadeUp` animations with `once: true`.

| File                | Section ID          | Purpose & Design Notes |
| ------------------- | ------------------- | ---------------------- |
| `AboutHero.tsx`     | `#about-hero`       | **Cinematic hero.** 7-layer parallax system: dark base → warm glow → cup image (`hero-tilted-cup.png` cut-out) → gradient overlay → CSS steam wisps → floating beans → editorial grid lines → text. **Fully responsive across 4 breakpoints:** Mobile (≤767px): cup moves below heading in normal document flow — never overlaps text. Tablet (768–1023px): cup anchored right below text. Laptop (1024–1279px): cup `absolute` right, 56vw. Desktop (≥1280px): cup `absolute` right, 50vw. 12 floating beans with independent parallax (`useTransform`), scroll-reactive opacity, max ~100° rotation. Steam (5 CSS wisps: `steam-wisp-1..5`) fades on scroll. Headline has minimal `7%` y-parallax. Bean count progressive: 5 on mobile → 7 on tablet → 12 on desktop. |
| `WhyXVIII.tsx`      | `#why-xviii`        | **Split layout — The Name.** Two-column grid (`lg:grid-cols-2`). Left: giant muted `XVIII` Roman numeral (`clamp(5rem, 15vw, 14rem)`, `#B8956A/15` opacity, `overflow-hidden`) with decorative top/bottom rules and "Est. XVIII Brew Co. · Kanpur, India" caption. Right: serif opening question, editorial body copy explaining the number eighteen, gold blockquote footnote. |
| `WhatWeBelieve.tsx` | `#what-we-believe`  | **Three values cards.** Dark glassmorphism cards (`#1e1812` bg, `#B8956A/10` border). Cards: `01 Quality Over Speed`, `02 Fresh Desserts`, `03 Made With Intention`. Each has numbered badge, diamond/circle icon, title, gold divider, description. Staggered entry animation. |
| `OurPromise.tsx`    | `#our-promise`      | **Full-width centred statement.** "No shortcuts." headline with serif gold accent word. Subheadlines: "We're not trying to become the biggest café. We're trying to become someone's favourite one." Supporting sentence below. Inline scroll-driven dot indicator strip at bottom. |
| `FounderNote.tsx`   | `#founder-note`     | **Split layout — editorial image + quote.** Left 45%: large dark coffee photography image. Right 55%: `FOUNDER'S NOTE` eyebrow, oversized serif quotation marks, italic quote ("The world moves fast. We don't."), supporting paragraph, horizontal rule, `— Founder / THE XVIII BREW CO.` attribution. |
| `AboutCTA.tsx`      | `#about-cta`        | **Closing call to action.** Mirrors the global `FinalCTASection` style. Two-column: left has large edgy headline "Ready for your first cup?"; right has body copy + two buttons (`EXPLORE MENU →` gold-outlined / `ORDER NOW` dark-outlined) + teaser label "SOMETHING HAS BEEN STEEPING.". |
